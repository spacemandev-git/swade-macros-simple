...
function printMessage(message, roll = null) {
    let options = {
        speaker: {
            actor: currentActor,
    ...
    if (roll) {
        options.roll = roll;
        options.type = CHAT_MESSAGE_TYPES.ROLL;
    }
...
async function commitAttack(params) {
    ...
    if (isValidConditions) {
        ...
        let rolls = diceResultPool.map(el => el.roll);
        let mergingPool = new DicePool({ rolls: rolls }).evaluate();
        let roll = Roll.create(mergingPool.formula).evaluate();
        roll.terms = [mergingPool]
        roll.results = [mergingPool.total];
        roll._total = mergingPool.total;
        roll._rolled = true;
        printMessage(chatMessage, roll);
    }
}// end commitAttack
...
async function damageResult(params) {
    ...
    diceResultPool.forEach((el) => {
        ...
        if (macroSettings.displayOption == 1) {
            ...
            armors.push(parseInt(weapon.data.data.ap == "" ? 0 : weapon.data.data.ap) * -1)
    ...
    let chatMessage = await renderTemplate("modules/swade-macros-simple/templates/macro-combat-flow/chat-damage-result.html", {
        weaponImg: weapon.data.img,
        weaponName: weapon.data.name,
        weaponAp: weapon.data.data.ap == "" ? 0 : weapon.data.data.ap,
